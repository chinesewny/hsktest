...
(function () {
  ...
  const FEMALE_ZH_VOICE_HINTS = ["tingting", "ting-ting", "mei-jia", "meijia", "sinji", "huihui", "yaoyao", "kangkang", "hanhan", "zhiwei", "yunxia", "xiaoxiao", "xiaoyi", "xiaomeng", "xiaoshuang", "xiaohan", "xiaomo", "xiaoxuan", "xiaorui", "xiaoqiu", "xiaochen", "female", "woman", "girl", "女"];
  ...
  const levenshtein = (a = "", b = "") => {
    ...
    for (let i = 1; i <= m; i++) {
      for (let j = 1; j <= n; j++) {
        if (a[i - 1] === b[j - 1]) dp[i][j] = dp[i - 1][j - 1];else dp[i][j] = 1 + Math.min(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1]);
      }
    }
    return dp[m][n];
  };
  ...
  const calcSpeechAccuracy = (transcript, expectedHanzi, expectedPinyin) => {
    ...
    const stripLatin = s => String(s || "").normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().replace(/[^a-z]/g, "");
    ...
    const sim = Math.max(containsBonus, stringSimilarity(tHanzi, eHanzi), stringSimilarity(tLatin, ePinyin));
  ...
  const calcLevel = totalXp => {
    ...
    return {
      level: lvl,
      currentXp: totalXp - used,
      nextXp: xpForLevel(lvl)
    };
  };
  ...
  const getPrimaryMeaning = (value = "") => {
    ...
    if (!raw) return "";
    const candidates = raw.split(/[;；]/).map(item => item.trim()).filter(Boolean).flatMap(item => item.split(/,/).map(part => part.trim()).filter(Boolean));
    const cleaned = candidates.map(item => item.replace(/\bWHO\b|\bSB\b|\bSTH\b/gi, "").replace(/\s+/g, " ").replace(/[.。]$/g, "").trim()).filter(Boolean);
  ...
  const sfxCorrect = () => {
    beep(880, 100);
    setTimeout(() => beep(1320, 180), 110);
  };
  const sfxWrong = () => {
    beep(220, 250, "sawtooth", 0.18);
  };
  const sfxLevelUp = () => {
    [523, 659, 784, 1047].forEach((f, i) => setTimeout(() => beep(f, 150), i * 130));
  };
  const sfxClick = () => {
    beep(660, 50, "square", 0.08);
  };
  window.U = {
    toISO,
    getDayOfWeek,
