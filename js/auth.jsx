...
function loadUsers() { return U.ls.get(USERS_KEY, {}); }
function saveUsers(u) { U.ls.set(USERS_KEY, u); }
function isOnlineAuth() { return !window.OFFLINE_MODE && !!window.fbAuth; }
...
function normalizeUser(user = {}) {
  ...
  return {
    ...
    items: user.items || { fifty: 1, time: 1, pinyin: 1, shield: 0, dice: 1, double: 0, ice: 0 },
    uid: user.uid || "",
...
function saveLocalUser(user) {
  ...
  users[user.studentId] = { ...(users[user.studentId] || {}), ...user };
  saveUsers(users);
...
async function fetchMemberByStudentId(studentId) {
  ...
  if (window.fbDB) {
    try {
      const doc = await withTimeout(
        window.fbDB.collection("members").doc(studentId).get(),
  ...
  try {
    const rows = await GSheets.read("Members", { keyField: "studentId", keyValue: studentId, limit: 1 });
...
async function fetchMemberByEmail(email) {
  ...
  if (window.fbDB) {
    try {
      const snap = await withTimeout(
        window.fbDB.collection("members").where("email", "==", email).limit(1).get(),
        FIRESTORE_READ_TIMEOUT_MS,
        "firestore read"
      );
  ...
  try {
    const rows = await GSheets.read("Members", { keyField: "email", keyValue: email, limit: 1 });
    if (rows[0]) return saveLocalUser(normalizeUser(rows[0]));
  } catch (e) {
    console.warn("[auth] sheets read failed", e.message);
  }
...
async function persistMemberProfile(user, { keyField = "studentId" } = {}) {
  ...
  if (window.fbDB) {
    try {
      await withTimeout(
        window.fbDB.collection("members").doc(normalized.studentId).set(normalized, { merge: true }),
  ...
  if (GSheets.shouldMirror()) {
    GSheets.enqueueUpsert("Members", { field: keyField, value: normalized[keyField] }, normalized).catch(() => {});
  }
  return normalized;
}
...
function PasswordInput({ value, onChange, placeholder = "", required = false, minLength, className = "" }) {
...
function SystemLogo({ size = "md", className = "" }) {
...
window.AuthService = {
  async register({ studentId, password, fullname, email, classroom, classNumber }) {
    ensureBootstrapAdmin();
    ...
    if (isOnlineAuth()) {
      try {
        const cred = await Promise.race([
          ...
          new Promise((_, rej) => setTimeout(() => rej(new Error("firebase timeout")), 3000))
        ]);
    ...
    const user = normalizeUser({
      studentId, fullname, email, classroom, classNumber,
      passHash, uid
    });
  ...
  async login(studentId, password) {
    ...
    if (isOnlineAuth() && u.email) {
      try {
        await Promise.race([
          window.fbAuth.signInWithEmailAndPassword(u.email, password),
          new Promise((_, rej) => setTimeout(() => rej(new Error("firebase timeout")), 3000))
        ]);
        onlineOk = true;
      } catch (e) {
  ...
  async hydrateCurrent() {
    ...
    if (email) {
      ...
      if (user) {
        const hydrated = normalizeUser({ ...user, uid: user.uid || firebaseUser?.uid || "" });
  ...
  async forgotPassword(studentId, email) {
    ...
    if (isOnlineAuth()) {
      ...
      return { mode: "email", message: "ระบบได้ส่งลิงก์รีเซ็ตรหัสผ่านไปที่อีเมลของคุณแล้ว" };
    }
    ...
    saveUsers(users);
    return { mode: "local", message: newPw };
  },
  ...
  updateUser(studentId, patch) {
    ...
    const updated = normalizeUser({ ...users[studentId], ...patch, updatedAt: new Date().toISOString() });
    users[studentId] = updated;
    saveUsers(users);
    if (window.fbDB) window.fbDB.collection("members").doc(studentId).set(updated, { merge: true }).catch(() => {});
    if (GSheets.shouldMirror()) {
      GSheets.enqueueUpsert("Members", { field: "studentId", value: studentId }, updated).catch(() => {});
    }
    return updated;
  },

  allUsers() { ensureBootstrapAdmin(); return Object.values(loadUsers()); },
  ...
  async fetchUsersByIds(studentIds = []) {
    ...
    uniqueIds.forEach((studentId) => {
      if (localUsers[studentId]) resultMap[studentId] = normalizeUser(localUsers[studentId]);
      else missingIds.push(studentId);
    });
  ...
  async fetchAllUsers() {
    ...
    try {
      ...
      if (window.fbDB) {
        const snap = await withTimeout(
          window.fbDB.collection("members").get(),
          FIRESTORE_READ_TIMEOUT_MS,
          "firestore read"
        );
...
function AuthCard({ children }) {
  ...
  return (
    <div className="scholar-shell arena-shell arena-grid arena-noise min-h-screen flex items-center justify-center p-4">
      <div className="arena-orb bg-emerald-300/20 h-40 w-40 left-[10%] top-[12%]"></div>
      <div className="arena-orb bg-amber-300/20 h-52 w-52 right-[8%] top-[18%]" style={{animationDelay:"1.3s"}}></div>
      <div className="arena-orb bg-rose-500/20 h-44 w-44 left-[20%] bottom-[10%]" style={{animationDelay:"2.1s"}}></div>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(20)].map((_,i)=>(
          <div key={i} className="absolute text-white/5 text-6xl font-bold animate-pulse font-zh"
               style={{left:`${Math.random()*100}%`, top:`${Math.random()*100}%`,
                       animationDelay:`${Math.random()*3}s`}}>
            {["汉","语","学","中","文","你","好","爱","学","习"][i%10]}
          </div>
        ))}
      </div>
      <div className="relative z-10 grid w-full max-w-6xl gap-8 lg:grid-cols-[1.05fr_.95fr] items-center">
        <div className="hidden lg:block">
          <div className="max-w-xl">
            <div className="arena-badge inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold">
              <span className="arena-signal">●</span> Chinese Scholar Arena
            </div>
            <h1 className="font-display mt-5 text-5xl font-bold leading-none text-[var(--arena-cream)]">
              HSK Vocabulary
              <span className="block text-[var(--arena-gold)]">Hanzi Academy 3.1</span>
            </h1>
            <p className="mt-5 text-lg text-[#5f4a43]">
              ฝึกคำศัพท์รายวัน ทบทวน 4 ทักษะ และสะสมคะแนนการแข่งขันในบรรยากาศห้องเรียนภาษาจีนที่ดูจริงจังแต่ยังสนุก
            </p>
            <div className="mt-8 grid grid-cols-3 gap-3">
              {[
                [`${game.daysPerWeek} วัน`, "รอบฝึก"],
                [`${game.questionsPerTest} ข้อ`, "รอบสอบ"],
                [`${game.seasonRounds} ครั้ง`, "ชิงแชมป์"]
              ].map(([n,l]) => (
                <div key={n} className="arena-panel rounded-2xl p-4">
                  <div className="font-display text-3xl font-bold text-[var(--arena-gold)]">{n}</div>
                  <div className="mt-1 text-sm text-[#6a544d]">{l}</div>
                </div>
              ))}
            </div>
          </div>
        </div>
        <div className="arena-panel relative w-full max-w-md overflow-hidden rounded-[28px] mx-auto">
          <div className="lacquer-panel p-6 text-center">
            <div className="font-display text-xs font-bold uppercase tracking-[0.28em] text-[var(--arena-gold-soft)]">Student Login Portal</div>
            <SystemLogo size="sm" className="mx-auto mt-3" />
            <h1 className="font-display text-3xl font-bold text-[var(--arena-paper)]">HSK Battle Arena</h1>
            <p className="mt-1 text-sm font-medium text-white/80">ระบบฝึกฝนคำศัพท์ภาษาจีน v3.1</p>
            <p className="mt-3 text-xs font-medium text-white/70">ห้องเรียนภาษาจีน โรงเรียนวังน้ำเย็นวิทยาคม</p>
          </div>
...
function LoginForm({ onSwitch, onSuccess }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      U.toast("ยินดีต้อนรับ " + u.fullname, "success"); U.sfxLevelUp();
      onSuccess(u);
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <h2 className="text-xl font-bold text-gray-800 text-center">เข้าสู่ระบบ</h2>
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">เลขประจำตัวนักเรียน หรือ admin</label>
        <input className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-rose-500 outline-none text-lg tracking-wider"
               maxLength="20" value={studentId}
               onChange={e=>setSid(e.target.value.trimStart())}
               placeholder="00000 หรือ admin" required />
      </div>
      <div>
      ...
      <div className="flex justify-between text-sm">
        <button type="button" onClick={()=>onSwitch("register")} className="text-rose-600 hover:underline">
          สมัครสมาชิกใหม่
        </button>
        <button type="button" onClick={()=>onSwitch("forgot")} className="text-gray-600 hover:underline">
          ลืมรหัสผ่าน?
        </button>
      </div>
    </form>
...
function RegisterForm({ onSwitch, onSuccess }) {
  const [f, setF] = useState({
    studentId:"", password:"", confirmPw:"", fullname:"", email:"",
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      <h2 className="text-xl font-bold text-gray-800 text-center">สมัครสมาชิกใหม่</h2>
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <label className="block text-xs font-semibold text-gray-600 mb-1">เลขประจำตัว (5 หลัก) *</label>
          <input className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-rose-500 outline-none"
                 maxLength="5" value={f.studentId}
                 onChange={e=>setField("studentId",e.target.value.replace(/\D/g,""))} required />
        </div>
        <div className="col-span-2">
          <label className="block text-xs font-semibold text-gray-600 mb-1">ชื่อ-นามสกุล *</label>
          <input className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-rose-500 outline-none"
                 value={f.fullname} onChange={e=>setField("fullname",e.target.value)} required />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">ชั้น *</label>
          <select className="w-full bg-white text-slate-900 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-rose-500 outline-none"
                  value={f.classroom} onChange={e=>setField("classroom",e.target.value)}>
            {window.APP_CONFIG.classes.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">เลขที่</label>
          <input className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-rose-500 outline-none"
                 value={f.classNumber} onChange={e=>setField("classNumber",e.target.value)} />
        </div>
        <div className="col-span-2">
          <label className="block text-xs font-semibold text-gray-600 mb-1">อีเมล (สำหรับลืมรหัสผ่าน) *</label>
          <input type="email" className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-rose-500 outline-none"
                 value={f.email} onChange={e=>setField("email",e.target.value)} required />
        </div>
        <div>
...
function ForgotForm({ onSwitch }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setResetResult(result); U.toast("รีเซ็ตรหัสผ่านสำเร็จ", "success");
  ...
  if (resetResult) return (
    <div className="text-center space-y-4">
      ...
      <h3 className="text-lg font-bold text-gray-800">
        {resetResult.mode === "email" ? "ส่งลิงก์รีเซ็ตรหัสผ่านแล้ว" : "รหัสผ่านใหม่ของคุณ"}
      </h3>
      {resetResult.mode === "local" && (
        <div className="bg-amber-50 border-2 border-amber-300 rounded-xl p-4">
          <code className="text-2xl font-mono font-bold text-amber-700">{resetResult.message}</code>
        </div>
      )}
      {resetResult.mode === "email" && (
        <div className="bg-emerald-50 border-2 border-emerald-300 rounded-xl p-4 text-sm text-emerald-800">
          {resetResult.message}
        </div>
      )}
      <p className="text-sm text-gray-600">
        {resetResult.mode === "email" ? "กรุณาเปิดอีเมลเพื่อกำหนดรหัสผ่านใหม่" : "กรุณาเปลี่ยนรหัสผ่านหลังเข้าสู่ระบบ"}
      </p>
      <button onClick={()=>onSwitch("login")}
              className="cinnabar-btn mobile-btn w-full rounded-xl py-3 font-bold">เข้าสู่ระบบ</button>
    </div>
  );

  return (
    <form onSubmit={submit} className="space-y-4">
      <h2 className="text-xl font-bold text-gray-800 text-center">ลืมรหัสผ่าน</h2>
      <p className="text-sm text-gray-600 text-center">กรอกเลขประจำตัวและอีเมลที่ลงทะเบียนไว้</p>
      <input className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-rose-500 outline-none"
             maxLength="5" value={studentId}
             onChange={e=>setSid(e.target.value.replace(/\D/g,""))}
             placeholder="เลขประจำตัวนักเรียน" required />
      <input type="email" className="w-full bg-white text-slate-900 placeholder:text-slate-400 caret-slate-900 px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-rose-500 outline-none"
             value={email} onChange={e=>setEmail(e.target.value)}
             placeholder="อีเมล" required />
      <button type="submit" className="cinnabar-btn mobile-btn w-full rounded-xl py-3 font-bold">รีเซ็ตรหัสผ่าน</button>
      <button type="button" onClick={()=>onSwitch("login")}
              className="w-full text-center text-gray-600 hover:underline text-sm">กลับไปหน้าเข้าสู่ระบบ</button>
    </form>
  );
}

window.AuthScreen = function AuthScreen({ onLogin }) {
  const [mode, setMode] = useState("login");
  return (
    <AuthCard>
      {mode === "login"    && <LoginForm    onSwitch={setMode} onSuccess={onLogin} />}
      {mode === "register" && <RegisterForm onSwitch={setMode} onSuccess={onLogin} />}
